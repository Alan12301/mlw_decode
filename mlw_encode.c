...
// An algorithm direct find the best case for a tile
// each tile fro only use one config
static int search_best_tile_grc_params( const int *inval_buf,
                              int tile_id,
                              int n_inval,
                              int zrun_mode,
                              int uncompressed_bits,
                              ...
                              int *bitcnt )
{
    ...
    // Loop over each config to get each bitcount
    int existing_idx=0;
    for(i=0; i<n_cfg; i++) {
        int value = inval_buf[i];
        ...
        int cmd_cost = 24;
        if (zrun_mode) {
            // free transition, because the weight stream already inserted a command at this tile
            bool change_w_config = tile_id == 0 ?  true : existing_grc_param[tile_id - 1] == existing_grc_param[tile_id] ? false : true;
            cmd_cost = change_w_config == true ? 0 : 24;
        }

        // Loop over inval_buf
        for(j=0; j<n_inval; j++) {
            int value = inval_buf[j];
            int div = grc_params[i]&15;
            int trunc = grc_params[i]>>4;
            int q = value>>div;
            int bits = trunc ? min(q+1,2) + div : q+1+div;
            if (!zrun_mode && ((trunc && q>2) || q>31))
                bits=10000;  // it's not possible to code the current value; give it a high cost
            ...
            state[i][0].bitcnt +=  bits;
        }

        int prev_config = tile_id == 0 ?  0 : grc_param_cfg[tile_id - 1];
        if(prev_config != i || tile_id == 0)
            state[i][0].bitcnt += cmd_cost;
    }

    // Best GRC parameter 
    int best_bitcnt=0x7fffffff, best_cfg=0;
    for(i=0; i<n_cfg; i++) {
        if (state[i][0].bitcnt < best_bitcnt) {
            best_bitcnt = state[i][0].bitcnt;
            best_cfg = i;
        }
    }
    

    grc_param_cfg[tile_id] = best_cfg;
    int endpos=n_inval;
    grc_param_pos[tile_id] = tile_id == 0 ? endpos : endpos - grc_param_pos[tile_id - 1];
                         ...
                         int verbose )
{
    ...
    // trunc mode not support in tile mode now
    // if(tile_mode)
    //     w_grc_trunc     = 0;
    ...
    int z_nvalues = tile_mode ? nvalues + 1 : nvalues + (new_palette?1:0);
    int w_pos=0, z_pos=0;
                           ...
                           int verbose )
{
    ...
    int *weight_values =0;
    int *zrun_values = 0;
    int *weight_values_tile_pos =0; 
    int *zrun_values_tile_pos =0; 

    double input_channel_d = input_channel;
    double output_channel_d = output_channel;
    int tile_i_num = ceil(input_channel_d / 32); //Record how many tiles there are in I dimension
    int tile_o_num = ceil(output_channel_d / 16); //Record how many tiles there are in O dimension
    int tile_num = 0;
    int tile_i_size = 0;
    int tile_o_size = 0;
    int tile_size = 0;
    ...
    int count = 0;
    int control_cnt = 2048; 
    do {
        CHECKED_MALLOC( weight_values, size*sizeof(int)*2 );
        CHECKED_MALLOC( zrun_values, size*sizeof(int)*2 );
        CHECKED_MALLOC( weight_values_tile_pos, size*sizeof(int)*2 );
        CHECKED_MALLOC( zrun_values_tile_pos, size*sizeof(int)*2 );

        // Get weights (or weight indicies) AND zero-runs from the input weight stream.
        int pos=0, n_weights = 0, zcnt=0, tile_n_zvalue = 0;
        if(!tile_mode){
            while(1) {
                if (p->use_zero_runs) {
                    zcnt=0;
                    ...
                    if (!p->only_zeros || pos>0) {
                        while( pos<size && inbuf[pos]==0) {
                            zcnt++;
                            pos++;
                        }
                    }
                    zrun_values[n_weights] = zcnt;
                    //printf("%d, ", zrun_values[n_weights]);
                }
                if (pos==size)
                    break;
                int value = p->inv_lut[inbuf[pos]+256];
            ...
            for(int tile_o_id = 1; tile_o_id <= tile_o_num; tile_o_id++ )
            {
                for(int tile_i_id = 1; tile_i_id <= tile_i_num; tile_i_id++ )
                {
                    for(int i = 1; i <= kernel_height * kernel_width; i++)
                    {
                        if (tile_o_id == tile_o_num)
                        {
                            tile_o_size = output_channel % 16;
                            if(tile_o_size == 0)
                            {
                                tile_o_size = 16;
                            }
                        ...
                        if (tile_i_id == tile_i_num)
                        {
                            tile_i_size = input_channel % 32;
                            if(tile_i_size == 0)
                            {
                                tile_i_size = 32;
                            }
                        ...
                        tile_size = tile_i_size * tile_o_size;
                        for(int j = 0; j < tile_size ; j++)
                        {
                            if (p->use_zero_runs) {
                                // Count zero run
                                // Special case: if all weights in the tile are zero, we must
                                // still ensure we have one coded weight so the the slice length
                                // doesn't become 0. Therefore we skip the first zero run and code
                                // the zero explicitly as a weight value instead
                                if( pos < size && inbuf[pos]==0 ) {
                                    zcnt++;
                                    if(pos == size - 1 || j == tile_size - 1){
                                        if(zcnt == tile_size){ 
                                            zrun_values[tile_n_zvalue] = 0;
                                            tile_n_zvalue++;
                                            ...
                                            zrun_values[tile_n_zvalue] = zcnt - 1;
                                            tile_n_zvalue++;
                                            zcnt = 0;

                                        }
                                        else{
                                            zrun_values[tile_n_zvalue] = zcnt;
                                            tile_n_zvalue++;
                                            zcnt = 0;
                                        }
                                    }
...
// ----------------------------------------------------------------------------
                            ...
                            if(count == control_cnt) {
                                weight_values_tile_pos[tile_num] = n_weights;
                                zrun_values_tile_pos[tile_num] = tile_n_zvalue;
                                tile_num++;
                                count = 0;
                            }
                            if(pos == size){
                                int remain_space = (tile_num + 1)*control_cnt- size ;
                                if(remain_space > 0) {
...
//-----------------------------------------------------------------------------
        ...
        int n_w_slice = 0, w_bitcnt = 0, total_w_bitcnt = 0, tile_id = 0, n_tile_weights = 0, w_start_pos = 0;
        ...
        if(!tile_mode){
            ...
            {
                w_start_pos = tile_id == 0 ? 0 : weight_values_tile_pos[tile_id - 1];
                n_tile_weights = weight_values_tile_pos[tile_id] - w_start_pos; 
                n_w_slice = n_w_slice + search_best_tile_grc_params( weight_values + w_start_pos, tile_id, n_tile_weights,
                                                                     0, uncompressed_bits, w_slice_cfg, w_slice_pos, size, 0, &w_bitcnt);
                total_w_bitcnt +=  w_bitcnt;
                int grc_div = w_grc_params[w_slice_cfg[tile_id]] & 15;
                int w_uncompressed  = (w_grc_params[w_slice_cfg[tile_id]] >> 4)==2;
                if (w_uncompressed) {
                    grc_div = uncompressed_bits;
                }
                int wdiv = !w_uncompressed ? grc_div : 7;
            }
        }
        ...
        int n_z_slice=0, z_bitcnt=0, total_z_bitcnt = 0, n_tile_zvalue = 0, z_start_pos = 0;
        if (p->use_zero_runs) {
            ...
            if(!tile_mode){
                ...
                for(tile_id = 0; tile_id < tile_num; tile_id++)
                {
                    z_start_pos = tile_id == 0 ? 0 : zrun_values_tile_pos[tile_id - 1];
                    n_tile_zvalue = zrun_values_tile_pos[tile_id] - z_start_pos; 
                    n_z_slice = n_z_slice + search_best_tile_grc_params( zrun_values + z_start_pos, tile_id, n_tile_zvalue,
                                                                         1, 0, z_slice_cfg, z_slice_pos, size, w_slice_cfg, &z_bitcnt);
                    total_z_bitcnt +=  z_bitcnt;
                }
                if ( n_z_slice < 0 ) {  // Memory allocation failed
                    bitpos = -1;
                    break;
        ...
        pos=0;
        int w_pos=0, z_pos=0, i_w_slice=0, i_z_slice=0, new_palette=1;
        if(!tile_mode){
            while(pos<n_weights || new_palette) {
                ...
                bitpos = encode_slice( weight_values+pos, zrun_buf, len,
                                    ...
                                    false, bitbuf, bitbuf_size, bitpos, verbose );
                new_palette = 0;
            ...
            for(int tile_id = 0; tile_id < tile_num; tile_id++){
                ...
                int w_len = weight_values_tile_pos[tile_id] - w_pos;
                int *zrun_buf = p->use_zero_runs ? zrun_values + z_pos : 0;

                bitpos = encode_slice( weight_values + w_pos, zrun_buf, w_len,
                                    p, new_palette, uncompressed_bits,
                                    w_slice_cfg[i_w_slice], p->use_zero_runs ? z_slice_cfg[i_z_slice] : 0,
                                    true, bitbuf, bitbuf_size, bitpos, verbose );

                new_palette = 0;

                w_pos = weight_values_tile_pos[tile_id];
                z_pos = zrun_values_tile_pos[tile_id];
                i_w_slice++;
                if(p->use_zero_runs)
                    i_z_slice++;
    ...
    free(weight_values);
    free(zrun_values);
    free(weight_values_tile_pos);
    free(zrun_values_tile_pos);
...
// Return -1 if error
int mlw_encode( int16_t *inbuf, int kernel_height, int kernel_width, int input_channel, int output_channel,
                int inbuf_size, uint8_t **outbuf, bool tile_mode, int **tile_endpos, int verbose)
                {
    int i;
#ifndef NDEBUG
    // Range check
    ...
    int bitbuf_size = inbuf_size*5+1024;
    assert(*outbuf == NULL);
    *outbuf = malloc( bitbuf_size );
    ...
    for(i=0; i<n_restarts && bitpos >= 0; i++) {
        ...
        find_palette( inbuf, inbuf_size, &palette); // change this 
        create_inverse_palette( &palette);
        bitpos = encode_section( inbuf+pos, kernel_height, kernel_width, 
                                input_channel, output_channel, tile_mode,
                                tile_endpos, size, &palette,*outbuf, 
                                bitbuf_size, bitpos, verbose );
    }


    // Add end of stream marker and align to 128bit
    int ret = -1;
    if ( bitpos >= 0 && n_restarts >= 0 ){
        bitbuf_t bitbuf_s, *bb=&bitbuf_s;
