...
int main(int argc, char *argv[])
{
    ...
    for(infile_idx=0; infile_idx<nbr_of_infiles; infile_idx++) {
        ...
        if (!decode) {
            ...
            int16_t *weights_256;
            MLWDecoder decoder;
            mlw_decoder_init(&decoder, inbuf, inbuf_size, &weights_256, verbose);
            outbuf_size = 0;
            // 使用解碼器來解碼數據
            int count2stop = 256;
            int count = 0;
            ...
            do{
                ...
                n = mlw_decode(&decoder, count2stop);
                total_output_size += n;
                weights = realloc( weights, total_output_size*sizeof(int16_t));
                for(int idx=0; idx < n; idx++){
                    weights[count*count2stop+idx] = weights_256[idx];
                }
                count++;
            }while (*decoder.outbuf && !decoder.end_decode);
            

            outbuf_size = int16_format ? total_output_size*(int)sizeof(int16_t) : total_output_size;
            outbuf = malloc( outbuf_size );
            assert(outbuf);
            for(i=0; i<total_output_size; i++) {
                if (int16_format)
            ...
            free(weights);
            printf("Input size %d output size %d bpw %4.2f\n", inbuf_size, total_output_size, total_output_size ? inbuf_size*8.0/total_output_size : 0);
